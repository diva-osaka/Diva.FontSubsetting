...
* サブセット化したフォントの PostScript 名を「{タグ}+{元の PostScript 名}」に変更する。[Name ID](https://learn.microsoft.com/en-us/typography/opentype/spec/name#name-ids) = 6 の値を変更する。
  * タグは suffix とサブセットに含める文字から生成する大文字6文字（PDF 仕様のフォントサブセット接頭辞）。内容が異なるサブセットには異なるタグが付く。SkiaSharp は PostScript 名を PDF の `/BaseFont` にそのまま使うため、これにより複数の PDF を結合しても別々のサブセットが同名にならない。
